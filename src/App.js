...
class App extends Component {
  state = {
    ...
    imageSrc: imagesList[0].imageUrl,
    time: 60,
    score: 0,
    isPlayAgain: false,
  }
  ...
  renderPlayGameView = () => {
    const {imageSrc, activeTabId} = this.state

    const categoriesList = imagesList.filter(
      eachItem => eachItem.category === activeTabId,
    )

    return (
      <>
        <img src={imageSrc} className="image" alt="match" />
        ...
        <ul className="itemsList">
          {categoriesList.map(eachItem => (
            <CategoryItem
              ...
              key={eachItem.id}
              updateImagSrcRandomly={this.updateImagSrcRandomly}
            />
          ))}
        </ul>
      </>
  ...
  updateImagSrcRandomly = thumbnailUrl => {
    ...
    if (imageSrc === imageUrl) {
      const newImageUrl =
        imagesList[Math.ceil(Math.random() * imagesList.length)].imageUrl
      this.setState({
  ...
  render() {
    const {time, score, isPlayAgain} = this.state
        ...
        <div className="header-container">
          ...
          <ul className="score-time-container">
            <li className="score-container">
              <p className="score">
                Score: <span className="countSpan">{score}</span>
              </p>
              <img
            ...
            <li>
              <p className="timeDuration">{time} sec</p>
            </li>
          </ul>
        </div>
        {isPlayAgain && this.renderGameOverView()}
        {!isPlayAgain && this.renderPlayGameView()}
      </div>
    )
  }
}
